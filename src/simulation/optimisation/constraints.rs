...
pub fn add_asset_constraints<'a, I>(
    ...
    model: &'a Model,
    assets: &I,
    commodities_to_balance: &'a [CommodityID],
    commodities_to_allow_unmet_demand: &'a [CommodityID],
    year: u32,
) -> ConstraintKeys
where
    ...
    I: Iterator<Item = &'a AssetRef> + Clone + 'a,
{
    ...
    for commodity_id in commodities_to_balance {
        ...
        for region_id in model.iter_regions() {
                ...
                .iter_selections_at_level(commodity.time_slice_level)
            {
                ...
                if commodities_to_allow_unmet_demand.contains(commodity_id) {
                    for (time_slice, _) in ts_selection.iter(&model.time_slice_info) {
                        let var =
