...
/// Similar to the above, but average across assets rather than taking the max for existing assets
/// Candidate assets are treated the same way (take the min across candidates)
    ...
    J: Iterator<Item = (&'a AssetRef, &'a TimeSliceID)>,
{
    // For each (commodity, region, group), accumulate per-asset marginal costs.
    // For seasonal/annual commodities, these marginal costs are weighted by activity.
    // Marginal costs for different assets are averaged according to activity.
    let mut existing_accum: IndexMap<_, WeightedAverageAccumulator> = IndexMap::new();
    for (asset, time_slice, activity) in activity_for_existing {
        let region_id = asset.region_id();
    ...
    // For each group, finalise per-asset weighted averages
    let group_prices: IndexMap<_, MoneyPerFlow> = existing_accum
        .into_iter()
...
where
    ...
    J: Iterator<Item = (&'a AssetRef, &'a TimeSliceID)>,
{
    // For each (commodity, region, group), accumulate full costs.
    // Costs are weighted across assets according to activity, and across time slices according to
    // activity for seasonal/annual commodities.
    let mut existing_accum: IndexMap<_, WeightedAverageAccumulator> = IndexMap::new();
    let mut annual_fixed_costs: HashMap<_, _> = HashMap::new();
    for (asset, time_slice, activity) in activity_for_existing {
        let annual_activity = annual_activities[asset];
        ...
        for (commodity_id, marginal_cost) in asset.iter_marginal_costs_with_filter(
            ...
            // Add marginal cost and fixed cost per flow, weighted by activity
            existing_accum
                .entry((commodity_id.clone(), region_id.clone(), group))
                .or_default()
                .add(
