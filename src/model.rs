...
use crate::commodity::{CommodityMap, MarketID};
...
impl Model {
    ...
    pub fn iter_markets(&self) -> impl Iterator<Item = MarketID> + '_ {
        self.commodities.keys().flat_map(move |commodity_id| {
            self.regions
                .keys()
                .map(move |region_id| MarketID::from((commodity_id, region_id)))
        })
    }
}
